...
tiDic = {
    ...
    "假性残疾": "假性残疾：调查员陷入了心理性的失明，失聪以及躯体缺失感中，持续 1D10 轮。",
    "暴力倾向": "调查员陷入了六亲不认的暴力行为中，对周围的敌人与友方进行着无差别的攻击，持续 1D10 轮。",
    "偏执": "调查员陷入了严重的偏执妄想之中，持续１Ｄ１０轮。有人在暗中窥视着他们，同伴中有人背叛了他们，没有人可以信任，万事皆虚。",
    "人际依赖": "人际依赖：守秘人适当参考调查员的背景中重要之人的条目，调查员因为一些原因而降他人误认为了他重要的人，并且努力的会与那个人保持那种关系，持续 1D10 轮",
    "昏厥": "调查员当场昏倒，并需要 1D10 轮才能苏醒。",
    "逃避行为": "调查员会用任何的手段试图逃离现在所处的位置，即使这意味着开走唯一一辆交通工具并将其它人抛诸脑后，调查员会试图逃离 1D10 轮。",
    "竭嘶底里": "调查员表现出大笑，哭泣，嘶吼，害怕等的极端情绪表现，持续 1D10 轮。",
    "恐惧": "调查员投一个 D100 或者由守秘人选择，来从恐惧症状表中选择一个恐惧源，就算这一恐惧的事物是并不存在的，调查员的症状会持续 1D10 轮。",
    "狂躁": "调查员投一个 D100 或者由守秘人选择，来从狂躁症状表中选择一个狂躁的诱因，这个症状将会持续 1D10轮。",
}
liDic = {
    "失忆": "回过神来，调查员们发现自己身处一个陌生的地方，并忘记了自己是谁。记忆会随时间恢复。",
    "被窃": "调查员在 1D10 小时后恢复清醒，发觉自己被盗，身体毫发无损。如果调查员携带着宝贵之物（见调查员背景），做幸运检定来决定其是否被盗。所有有价值的东西无需检定自动消失。",
    "遍体鳞伤": "调查员在 1D10 小时后恢复清醒，发现自己身上满是拳痕和瘀伤。生命值减少到疯狂前的一半，但这不会造成重伤。调查员没有被窃。这种伤害如何持续到现在由守秘人决定。",
    ...
    "重要之人": "考虑调查员背景中的重要之人，及其重要的原因。在 1D10 小时或更久的时间中，调查员将不顾一切地接近那个人，并为他们之间的关系做出行动。",
    "被收容": "调查员在精神病院病房或警察局牢房中回过神来，他们可能会慢慢回想起导致自己被关在这里的事情。",
    "逃避行为": "调查员恢复清醒时发现自己在很远的地方，也许迷失在荒郊野岭，或是在驶向远方的列车或长途汽车上。",
    "恐惧": "调查员患上一个新的恐惧症。在恐惧症状表上骰 1 个 D100 来决定症状，或由守秘人选择一个。调查员在 1D10 小时后回过神来，并开始为避开恐惧源而采取任何措施。",
    "狂躁": "调查员患上一个新的狂躁症。在狂躁症状表上骰 1 个 D100 来决定症状，或由守秘人选择一个。调查员会在 1d10 小时后恢复理智。在这次疯狂发作中，调查员将完全沉浸于其新的狂躁症状。这是否会被其他人理解（apparent to other people）则取决于守秘人和此调查员。",
}
obsessionsDic = {
    ...
    16: "隐居癖/恋车癖（Automania）：过度地热爱独自隐居。",
    ...
    83: "背德癖/漂泊症（Planomania）：病态地渴求违背社会道德",
    ...
    100: "喜兽癖（Zoomania）：对待动物的态度近乎疯狂的友好",
}
phobiaDic = {
...
# 临时/即时症状
async def getTi(msgStr):
    if msgStr == "list":
        # 全部症状
        resultList = []
        for a, b in tiDic.items():
            resultList.append(f"{a}: {b}")
        result = "\n".join(resultList)
        resultMsg = f"全部临时/即时症状如下：\n{result}"
        return resultMsg
    if msgStr in tiDic:
        resultMsg = f"{msgStr}: {tiDic[msgStr]}"
        return resultMsg
    symptomType = await roll(10)
    symptomTypeStr = f"1d10={symptomType}"
    i = 1
    des = ""
    for a, b in tiDic.items():
        if i == symptomType:
            des = f"{a}: {b}"
        i += 1
    durationTime = await roll(10)
    duration = f"1d10={durationTime}"

    resultMsg = (f"调查员陷入了临时/即时疯狂症状({symptomTypeStr})：\n{des}"
                 f"\n持续时间：{duration}")
    return resultMsg


# 总结症状
async def getLi(msgStr):
    if msgStr == "list":
        # 全部症状
        resultList = []
        for a, b in liDic.items():
            resultList.append(f"{a}: {b}")
        result = "\n".join(resultList)
        resultMsg = f"全部总结症状如下：\n{result}"
        return resultMsg
    if (msgStr == "焦躁症"
            or msgStr == "焦躁"
            or msgStr == "o"):
        # 全部症状
        resultList = []
        for a, b in obsessionsDic.items():
            resultList.append(f"{a}: {b}")
        result = "\n".join(resultList)
        resultMsg = f"全部焦躁症状如下：\n{result}"
        return resultMsg
    if (msgStr == "恐惧症"
            or msgStr == "恐惧"
            or msgStr == "p"):
        # 全部症状
        resultList = []
        for a, b in phobiaDic.items():
            resultList.append(f"{a}: {b}")
        result = "\n".join(resultList)
        resultMsg = f"全部恐惧症状如下：\n{result}"
        return resultMsg
    if msgStr in liDic:
        resultMsg = f"{msgStr}: {liDic[msgStr]}"
        return resultMsg
    symptomType = await roll(10)
    symptomTypeStr = f"1d10={symptomType}"
    i = 1
    ...
    if (symptomType == 1
        ...
        # durationTime = 0
        ext = ""
    elif symptomType == 9 or symptomType == 10:
        durationTime = await roll(10)
        durationStr = f"1d10={durationTime}"
        symptom = f"{symptom}症"
        symptomResult = await roll(100)
        symptomResultStr = f"1d100={symptomResult}"
        summary = ""
        if symptomType == 9:
            obsessions = obsessionsDic[symptomResult]
            summary = obsessions
        if symptomType == 10:
            phobia = phobiaDic[symptomResult]
            summary = phobia

        ext = (f"\n持续时间：{durationStr}"
               f"\n{symptom}({symptomResultStr})：{summary}")

    else:
        durationTime = await roll(10)
        durationStr = f"1d10={durationTime}"
        ext = f"\n持续时间：{durationStr}"
    resultMsg = f"调查员的总结症状为({symptomTypeStr})：\n{des}{ext}"
    return resultMsg
